// =========================================================
// 1. BASE DE DATOS (SIMULACIÓN DE DATOS DEL BACKEND)
// =========================================================
const baseDeDatosProfesores = [
  ...
        id: 8,
        nombre: "Julia Romero",
        imagen: "https://media.vogue.mx/photos/5c0702987ce4111b8fae953f/2:3/w_2560%2Cc_limit/amal_clooney_1346.jpg",
  ...
  // =========================================================
  // 2. SELECCIÓN DE ELEMENTOS (PANTALLA 5 y PANTALLA 6)
  // =========================================================
  const pantalla5 = document.getElementById('pantalla5');
  const pantalla6 = document.getElementById('pantalla6');
  const btnVolver = document.getElementById('btnVolver');
  
  // Selecciona todas las tarjetas de profesor para añadir el 'click'
  const todosLosProfesoresCards = document.querySelectorAll('.profesor');
  
  // =========================================================
  // 3. LISTENERS (MANEJO DE EVENTOS)
  // =========================================================
  
  // Listener para cada tarjeta de profesor (PANTALLA 5)
  todosLosProfesoresCards.forEach(card => {
    card.addEventListener('click', () => {
        // Captura el ID desde el atributo data-id-profesor del HTML
        const idProfesorClickeado = card.dataset.idProfesor;
        
  
        // Llama a la función que rellena y muestra la Pantalla 6
        mostrarDetalleProfesor(idProfesorClickeado);
    });
  });
  
  // Listener para el botón "Volver" (PANTALLA 6)
  ...
  /**
   * Rellena la Pantalla 6 con la información del profesor seleccionado.
   * @param {string} id - El ID del profesor.
   */
  function mostrarDetalleProfesor(id) {
    
    // Busca el objeto del profesor en el array por su ID
  ...
    // ----------------------------------------------------
    // 4.1 RELLENO DE LA PANTALLA 6 CON LOS DATOS DEL PROFESOR
    // ----------------------------------------------------
    
  ...
    // Relleno de las reseñas
    const listaReseñasHTML = document.getElementById('detalle-reseñas');
    listaReseñasHTML.innerHTML = ''; // Limpia reseñas anteriores
    
  ...
    // ----------------------------------------------------
    // 4.2 MANEJO DE LA VISIBILIDAD (OCULTAR/MOSTRAR)
    // ----------------------------------------------------
  ...
    // Opcional: Mover la vista al inicio de la página para que el usuario vea el detalle.
    window.scrollTo(0, 0);
  }
  
  /**
   * Muestra la Pantalla 5 (lista de profesores) y oculta la Pantalla 6.
   */
  ...
    
    // Mover la vista al inicio de la página.
    window.scrollTo(0, 0);
  }