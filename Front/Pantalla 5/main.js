// =================================================================
// ⭐ 1. FUNCIÓN DE COMUNICACIÓN CON EL BACKEND (para Soquetic)
// =================================================================

function enviarEventoGET(evento, callback) {
    const url = `http://localhost:3001/${evento}`;
    ...
    fetch(url)
        ...
        .then(data => {
            callback({ success: true, profesores: data });
        })
        .catch(error => {
            console.error("Error de comunicación con el Backend:", error);
            callback({ success: false, error: "No se pudo conectar con el servidor." });
        });
}
...
let baseDeDatosProfesores = []; // Aquí se guardará el JSON de profesores
...
const USUARIO_ACTUAL = "Tú (Alumno)"; // En el futuro vendrá del login


// =================================================================
// ⭐ 3. FUNCIONES DE LÓGICA DE LA PANTALLA
// =================================================================
...
function mostrarDetalleProfesor(id) {
    ...
    if (btnContactar) {
        btnContactar.onclick = () => {
            ...
            window.location.href = `../Pantalla 8/index.html?profesorNombre=${nombreCodificado}&profesorId=${profesor.id}`; 
        };
    }
...
// =================================================================
// ⭐ 4. FUNCIÓN DE INICIALIZACIÓN (Carga de Datos del Backend Soquetic)
// =================================================================

function inicializarPantalla() {
    enviarEventoGET('profesores', (respuestaBackend) => { 
        if (respuestaBackend.success) {
            baseDeDatosProfesores = respuestaBackend.profesores;
            cargarGaleria(); 
        } else {
            console.error("Error al cargar la BBDD de profesores:", respuestaBackend.error);
            galeriaProfesores.innerHTML = '<p>⚠️ Error al cargar la lista de profesores desde el servidor.</p>';
        }
    });
}
